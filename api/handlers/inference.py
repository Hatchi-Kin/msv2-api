...
async def get_embeddings_handler(
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.info(f"Successfully got embeddings for {audio_minio_path}: shape {result.shape}")
            return result
            ...
            logger.error(f"Inference service error {e.response.status_code}: {err_detail}")
            raise APIException(
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"Failed to reach inference service after {max_retries} attempts")
                raise APIException(
