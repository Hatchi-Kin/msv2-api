...
from api.agents.gem_hunter.state_v3 import AgentState, Track
...
class ToolNodes:
        ...
        try:
            ...
            if track_count < 5:
                return {
                    ...
                    "ui_state": {
                        ...
                        "options": [],
                        "cards": [],
                        "thought_process": [f"Playlist too small: {track_count} tracks"]
                    }
                }
            ...
            return {
                ...
                "ui_state": {
                    ...
                    "options": options,
                    "cards": [],
                    "thought_process": [f"Analyzed playlist with {stats.get('track_count', 0)} tracks"]
                }
            }
    ...
    async def search_tracks(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            ...
            excluded = state.get("known_artists", [])
            params = state.get("tool_parameters", {})
            
            # Build constraints based on vibe
            profile = state.get("playlist_profile", {})
            ...
            if iteration > 1:
                ...
                if "max_energy" in constraints:
                    constraints["max_energy"] = min(1.0, constraints["max_energy"] + 0.1)
    ...
    async def evaluate_results(self, state: AgentState) -> Dict[str, Any]:
        ...
        distances = [t.get("distance", 0.5) for t in candidates if isinstance(t, dict)]
        avg_distance = sum(distances) / len(distances) if distances else 0.5
        quality_score = 1.0 - avg_distance
        
        # Diversity score
        artists = set(t.get("artist") if isinstance(t, dict) else t.artist for t in candidates)
        ...
        logger.info(f"📊 Quality: {overall:.2f} ({'sufficient' if overall > 0.6 else 'insufficient'})")
    ...
    async def present_results(self, state: AgentState) -> Dict[str, Any]:
        ...
        try:
            ...
            unknown = [t for t in candidates if (t.get("artist") if isinstance(t, dict) else t.artist) not in known]
            ...
            for t in final:
                ...
                t_bpm = t.get("bpm") if isinstance(t, dict) else t.bpm
                t_energy = t.get("energy") if isinstance(t, dict) else t.energy
                ...
                try:
                    ...
                    if t_bpm and profile.get("avg_bpm"):
                        ...
                        if abs(t_bpm - avg_bpm) < 10:
                            comparisons.append(f"its {t_bpm:.0f} BPM perfectly matches your playlist's {avg_bpm:.0f} BPM tempo")
                        elif t_bpm < avg_bpm:
                            comparisons.append(f"its slower {t_bpm:.0f} BPM (vs your {avg_bpm:.0f}) creates a more relaxed feel")
                        else:
                            comparisons.append(f"its faster {t_bpm:.0f} BPM (vs your {avg_bpm:.0f}) adds subtle energy")
                    ...
                    if t_energy is not None and profile.get("avg_energy") is not None:
                        ...
                        if abs(t_energy - avg_energy) < 0.1:
                            comparisons.append(f"energy level of {t_energy:.2f} closely matches your {avg_energy:.2f}")
                        elif t_energy < avg_energy:
                            comparisons.append(f"lower energy ({t_energy:.2f} vs {avg_energy:.2f}) maintains the intimate vibe")
                        else:
                            comparisons.append(f"higher energy ({t_energy:.2f} vs {avg_energy:.2f}) adds dynamic contrast")
                    ...
                    if t_brightness and profile.get("avg_brightness"):
                        ...
                        if abs(t_brightness - avg_brightness) < 200:
                            comparisons.append(f"brightness of {t_brightness:.0f} matches your {avg_brightness:.0f} tonal palette")
                        elif t_brightness < avg_brightness:
                            comparisons.append(f"warmer tones ({t_brightness:.0f} vs {avg_brightness:.0f}) deepen the atmosphere")
                        else:
                            comparisons.append(f"brighter tones ({t_brightness:.0f} vs {avg_brightness:.0f}) add clarity")
                    
                    # Harmonic ratio (show the numbers!)
                    if t_harmonic_ratio is not None and profile.get("avg_harmonic_ratio") is not None:
                        avg_harmonic = profile["avg_harmonic_ratio"]
                        if abs(t_harmonic_ratio - avg_harmonic) < 0.1:
                            comparisons.append(f"harmonic ratio of {t_harmonic_ratio:.2f} aligns with your {avg_harmonic:.2f}")
                        elif t_harmonic_ratio > avg_harmonic:
                            comparisons.append(f"richer harmonics ({t_harmonic_ratio:.2f} vs {avg_harmonic:.2f}) add complexity")
                    
                    # Key (always show if available)
                    if t_key:
                        comparisons.append(f"composed in {t_key}")
                    
                    context = "; ".join(comparisons) if comparisons else "unique sonic qualities"
            ...
            try:
                ...
                if profile.get("top_genres"):
                    profile_desc.append(f"{', '.join(profile['top_genres'][:2])} influences")
                
                profile_str = "; ".join(profile_desc) if profile_desc else "your playlist's unique character"
                ...
                understanding_text = "Your playlist has a unique character that I've analyzed carefully."
                selection_text = f"I found {len(cards)} tracks that perfectly complement your vibe!"
