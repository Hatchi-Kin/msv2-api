...
async def analyze_playlist_stats(pool: asyncpg.Pool, playlist_id: int) -> Dict[str, Any]:
...
def filter_tracks_strict(
    ...
    for t in tracks:
        ...
        if t.bpm is not None:
            if t.bpm < constraints.get("min_bpm", 0): continue
            if t.bpm > constraints.get("max_bpm", 999): continue
            
        # Check Energy
        if t.energy is not None:
            if t.energy < constraints.get("min_energy", 0): continue
            if t.energy > constraints.get("max_energy", 1.0): continue
